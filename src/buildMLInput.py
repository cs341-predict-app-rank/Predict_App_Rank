...
def buildMatrix(object):
    ...
    def compressMatrix(rawData, windowSize = self.slidingWindowSize * self.WEEK, skipDay = self.WEEK):
        """
    ...
    def swipeOutInactiveApp(downloadMatrix, predictMatrix, leastDownload = self.garbageThreshold):
        """
    ...
    def generateAccumulateLabelByCol(dataMatrix, numberSigma = self.outOfSigmaSuccess):
        """
    ...
    def singlePredictTime(totalDataMatrix, predictTimeStamp,
            windowSize = self.slidingWindowSize,
            featureSize = self.featureTimeWindow - self.slidingWindowSize + 1,
            predictSize = self.predictTimeWindow - self.slidingWindowSize + 1,
            success = self.successThreshold):
        """
    ...
    def generateFeatureMatrixAndLabel(totalDataMatrix,
            windowSize = self.slidingWindowSize,
            featureWindow = self.featureTimeWindow,
            predictWindow = self.predictTimeWindow,
            success = self.successThreshold):
        """
    ...
    def buildMatrix(filename = self.inputFile):
        """
    ...
    def plotDownloadInflation(filename = self.inputFile):
        """
    ...
    def plotDownloadDistribution(filename = self.inputFile):
        """
    ...
    def randomPlot(filename = self.inputFile):
        rawData = rawDataMatrix(filename)
        rowNum = random.randint(1, rawData.shape[0]) - 1
        while rawData[rowNum].sum() < 100000:
            rowNum = random.randint(1, rawData.shape[0]) - 1
        plt.plot(standardize(rawData[rowNum]))
        plt.show()
